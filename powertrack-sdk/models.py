...
class SiteOverview:
    ...
    calculated_inverter_availability: float
    capacity_dc: float
    charge_discharge: Optional[float]
    custom_column_data: List[str]
    downtime_loss: float
    energy_availability: float
    energy_availability_loss: float
    energy_loss: float
    energy_ratio: float
    grid_offline: int
    ground: int
    ...
    lifetime: int
    message: str
    monitored_site_type: int
    parent_key: str
    payment_status: int
    performance_index: float
    performance_test_delta: float
    performance_test_status: int
    performance_test_value: float
    power: float
    power24: int
    power24_est: float
    power_avg15: float
    power_avg15_exp: float
    pv_capacity_ac: float
    pv_capacity_dc: float
    rated_power: Optional[float]
    reminder_color: str
    revenue_loss: float
    rolling24_kw: List[int]
    rolling24_kw_idx: int
    rule_tool_summary: Dict[str, Any]
    size_dc: float
    size_kw: float
    soiling_loss: float
    ...
    today_percent: float
    type: int
    today_and7_day_average_kw: Dict[str, Any]
    estimated_commissioning_date: Optional[str] = None
    ...
    def has_alerts(self) -> bool:
        ...
        return self.inverter_faults > 0

    @property
    def performance_status(self) -> str:
        """Get performance status based on energy ratio."""
        if self.energy_ratio >= 0.95:
            return "excellent"
        elif self.energy_ratio >= 0.85:
            return "good"
        elif self.energy_ratio >= 0.75:
            return "fair"
        else:
            return "poor"
...
class PortfolioMetrics:
    ...
    def total_capacity_ac(self) -> float:
        ...
        return sum(site.pv_capacity_ac for site in self.sites)

    @property
    def total_capacity_dc(self) -> float:
        """Total DC capacity across all sites."""
        return sum(site.pv_capacity_dc for site in self.sites)
...
class ChartSeries:
    ...
    def data_points(self) -> List[Tuple[int, float]]:
        ...
        return [(point['x'], point['y']) for point in self.data_xy if 'x' in point and 'y' in point]
